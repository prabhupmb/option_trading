...
const SignalCard: React.FC<SignalCardProps> = ({ signal, accessLevel }) => {
    ...
    const getSignalColor = (type: string, confidence: string) => {
        switch (type) {
            case 'BUY':
                return confidence === 'strong' ? 'bg-green-600 text-white' : 'bg-green-500/20 text-green-400 border border-green-500/30';
            case 'SELL':
    ...
    const getTrendIcon = (status: string) => {
        ...
        return 'trending_flat'; // consolidation
    };
    ...
    return (
        <div
            className={`bg-[#1a1f2e] border border-gray-800 rounded-xl overflow-hidden transition-all duration-300 hover:border-gray-600 ${expanded ? 'ring-1 ring-blue-500/50' : ''}`}
        >
            {/* Header */}
            <div className="p-5 cursor-pointer" onClick={() => setExpanded(!expanded)}>
                <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center bg-gray-800 border border-gray-700`}>
                            {/* Placeholder Icon or Logo logic */}
                            <span className="text-xs font-black text-gray-400">{signal.symbol[0]}</span>
                        </div>
                        <div>
                    ...
                    <div className="text-right">
                        <div className="text-2xl font-mono font-bold text-white tracking-tight">{formatCurrency(signal.current_price)}</div>
                        <div className="text-xs text-gray-500 font-medium">Current Price</div>
                    </div>
                </div>

                {/* Key Metrics Grid */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-3 border-y border-gray-800/50">
                    {signal.signal_type === 'BUY' && (
                        <>
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-sm font-mono text-blue-400">{signal.risk_reward_ratio || '-'}</p>
                            </div>
                        </>
                    )}
                    ...
                    {(signal.signal_type === 'SELL' || signal.signal_type === 'HOLD' || signal.signal_type === 'WAIT') && (
                        <>
                            ...
                            <div>
                                ...
                                <p className={`text-sm font-mono ${(signal.indicators?.rsi || 50) > 70 ? 'text-red-400' : (signal.indicators?.rsi || 50) < 30 ? 'text-green-400' : 'text-white'}`}>
                                    {signal.indicators?.rsi?.toFixed(0) || '-'}
                                </p>
                            </div>
                            <div>
                                <span className="text-[10px] text-gray-500 uppercase font-bold">ADX</span>
                                <p className="text-sm font-mono text-white">{signal.indicators?.adx?.toFixed(0) || '-'}</p>
                            </div>
                            <div className="text-right">
                                <span className="text-[10px] text-gray-500 uppercase font-bold">Support</span>
                                <p className="text-sm font-mono text-blue-400">{formatCurrency(signal.entry_price)}</p> {/* Using entry as support proxy if needed or null */}
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    {signal.indicators?.rsi && (
                        <span className="px-2 py-1 bg-gray-800 text-gray-400 text-[10px] font-mono rounded border border-gray-700">RSI {signal.indicators.rsi.toFixed(0)}</span>
                    )}
                    {signal.indicators?.adx && (
                        <span className="px-2 py-1 bg-gray-800 text-gray-400 text-[10px] font-mono rounded border border-gray-700">ADX {signal.indicators.adx.toFixed(0)}</span>
                    )}
                    {signal.indicators?.vwap && (
                        <span className="px-2 py-1 bg-gray-800 text-gray-400 text-[10px] font-mono rounded border border-gray-700">VWAP {signal.indicators.vwap}</span>
                    )}
                    {signal.confidence === 'strong' && (
                        <span className="px-2 py-1 bg-purple-500/10 text-purple-400 text-[10px] font-mono rounded border border-purple-500/20">HIGH CONVICTION</span>
                    )}
                </div>
            </div>
            ...
            {/* Expanded Content */}
            {expanded && (
                <div className="bg-[#151925] border-t border-gray-800 p-5 animate-in slide-in-from-top-2">

                    {/* When to Buy */}
                    <div className="mb-4 bg-blue-900/10 border border-blue-500/20 p-3 rounded-lg">
                        <h4 className="text-blue-400 text-xs font-bold uppercase tracking-widest mb-1 flex items-center gap-2">
                            <span className="material-symbols-outlined text-sm">lightbulb</span>
                            Action Plan
                        </h4>
                        <p className="text-gray-300 text-sm">{signal.when_to_buy}</p>
                    </div>
            ...
            <div className="flex border-t border-gray-800 divide-x divide-gray-800">
                <button className="flex-1 py-3 hover:bg-white/5 text-gray-400 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors">
                ...
                {accessLevel === 'signal' && (
                    <button className="flex-1 py-3 hover:bg-white/5 text-blue-400 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors">
                        <span className="material-symbols-outlined text-sm">notifications</span>
                        Set Alert
                    </button>
                )}

                <button className="flex-[0.5] py-3 hover:bg-red-500/10 hover:text-red-500 text-gray-500 text-xs font-bold uppercase tracking-wider flex items-center justify-center transition-colors">
