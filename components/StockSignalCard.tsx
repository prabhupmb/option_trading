...
const StockSignalCard: React.FC<Props> = ({ signal, onViewAnalysis, onExecute, accessLevel = 'signal' }) => {
  ...
  const getSignalBadgeStyle = (text: string) => {
    ...
    if (text === 'BUY' || text.includes('WEAK BUY')) return 'bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/30';
    if (text === 'SELL' || text.includes('WEAK SELL')) return 'bg-red-500/10 text-red-600 dark:text-red-400 border-red-500/30';
  ...
  const adxColor = (trend: string) => {
    switch (trend) {
      case 'VERY_STRONG': return 'text-green-400 font-black';
  ...
  return (
    <div className={`bg-white dark:bg-[#1a1f2e] border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden relative group transition-all duration-300 hover:border-gray-400 dark:hover:border-gray-600 hover:shadow-2xl ${isNoTrade ? 'opacity-60 grayscale-[0.5]' : ''}`}>
      ...
      {/* Header */}
      <div className="p-5 border-b border-gray-200/50 dark:border-gray-800/50 relative">
        <div className="flex justify-between items-start mb-2">
          <div className="flex items-center gap-3">
            <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter">{signal.symbol}</h3>
            <span className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded border ${signal.option_type === 'CALL' ? 'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400 border-green-300 dark:border-green-800' : 'bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-300 dark:border-red-800'}`}>
              {signal.option_type}
            </span>
          </div>
